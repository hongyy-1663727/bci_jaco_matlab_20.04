...
class DiscreteActionsRobot():
    ...
    def vision_inference_step(self):
        ...
        cv2.imshow("YOLO Handle Detection", annotated)
    ...
    def graspRoutine(self):
        ...
        if self.graspState == 0:
            if openRoutine  == 0: # drawer
                ...
                # quaternion  = tf.transformations.quaternion_from_euler(q[0], 0, quat_val, 'rxyz')  
                ...
                position =  self.goalObjs[self.currentGoal].SP[5]
                result      = cartesian_pose_client(position, quaternion, self.prefix)

                position =  self.goalObjs[self.currentGoal].SP[6]
                result      = cartesian_pose_client(position, quaternion, self.prefix)

                position =  self.goalObjs[self.currentGoal].SP[7]
                result      = cartesian_pose_client(position, quaternion, self.prefix)

                position =  self.goalObjs[self.currentGoal].SP[8]
                result      = cartesian_pose_client(position, quaternion, self.prefix)
                
                position =  self.goalObjs[self.currentGoal].SP[9]
                result      = cartesian_pose_client(position, quaternion, self.prefix)
